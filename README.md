...
(`~/.local/state/puntu/engine.log`, look for `client=…`) and add it to the list. Like every
setting, it takes effect within about a second — `config.toml` is hot-reloaded, no restart.
